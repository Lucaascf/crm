...
const conversationHandler = createConversationHandler(client, {
  testContacts: TEST_CONTACTS,
  botEnabledForAll: BOT_ENABLED_FOR_ALL,
})
...
// A sessão já ficou presa em "Loading your chats..." (observado tanto
// parado em 100% quanto em 99%, sem nunca disparar 'ready') — um reload da
// página destravou na hora nos dois casos. Detecta travamento em QUALQUER
// porcentagem: cada evento loading_screen rearma um timer de
// STUCK_TIMEOUT_MS; se STUCK_TIMEOUT_MS se passar sem nenhum evento NOVO
// (ou seja, sem progresso), recarregamos a página sozinhos (bem antes do
// LOGOUT de ~3min06s que uma sessão travada costumava sofrer).
...
client.on('loading_screen', (percent, message) => {
  ...
  stuckTimer = setTimeout(async () => {
    ...
    reloadAttempts++
    console.log(`[${new Date().toISOString()}] ⚠️ Travado em ${percent}% há ${STUCK_TIMEOUT_MS / 1000}s sem progresso — recarregando página (tentativa ${reloadAttempts}/${MAX_RELOAD_ATTEMPTS})`)
    try {
      await pupPage.reload({ waitUntil: 'load' })
      // o reload dispara uma sequência nova de loading_screen sozinho, que
      // rearma o timer de novo — não precisa fazer nada aqui além de deixar
      // reloadAttempts contado, pra não entrar num loop infinito de reload.
    } catch (err) {
...
// Usa 'message_create' (não 'message') de propósito: whatsapp-web.js só
// emite 'message' pra mensagens de terceiros (a própria lib descarta
// fromMe=true antes de emitir esse evento — ver node_modules/whatsapp-web.js
// /src/Client.js). Isso deixava o handoff humano impossível de implementar
// com 'message': o bot literalmente nunca via as mensagens que o operador
// mandava manualmente pelo mesmo número. 'message_create' emite pras duas
// direções; toda a lógica de filtragem/handoff/extração mora em
// src/conversationHandler.js (testável sem uma sessão WhatsApp de verdade).
client.on('message_create', (msg) => {
  conversationHandler.handleIncomingMessage(msg).catch((err) => {
    console.log(`[${new Date().toISOString()}] ⚠️ Falha ao processar mensagem: ${err.message}`)
  })
})
